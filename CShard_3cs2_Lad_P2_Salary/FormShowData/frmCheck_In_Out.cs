...
namespace CShard_3cs2_Lad_P2_Salary
{
    public partial class frmCheck_In_Out : Form
    {
        ...
        private void Check_IN()
        {
            try
            {
                if (txtID.Text != "" && txtNameLao.Text != "" && txtSureLao.Text != "" && txtTel.Text != "" && txtemail.Text != "") 
                {
        ...
        private void txtID_TextChanged(object sender, EventArgs e)
        {
            try
            {
                da = new SqlDataAdapter("Select Name_Lao, Surname_Lao, Name_Eng, Surname_Eng, mobile, email From tbStaff Where St_ID='" + txtID.Text + "'", con);
                table = new DataTable();
                da.Fill(table);
                if (table.Rows.Count > 0)
                {
                    txtNameLao.Text = table.Rows[0][0].ToString();
                    txtSureLao.Text = table.Rows[0][1].ToString();
                    txtNameEng.Text = table.Rows[0][2].ToString();
                    txtSureEng.Text = table.Rows[0][3].ToString();
