...
namespace CShard_3cs2_Lad_P2_Salary
{
    public partial class frmStaff : Form
    {
        ...
        public void ShearchData(string data)
        {
            try
            {
                da = new SqlDataAdapter("Select * From vw_Staff Where Name_Lao Like N'" + data + "%' Or Surname_Lao like N'%" + data + "%' Or Name_Eng like N'%" + data + "%' Or Surname_Eng like N'%" + data + "%' ", conn);
                ds = new DataSet();
                da.Fill(ds, "st");
                dgvStaff.DataSource = ds.Tables["st"];
                for (int c = 0; c < col.Length; c++)
